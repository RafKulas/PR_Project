...
void send_structure(int sock, game_object_t* pdata)
{
    ...
    {
        buffer[PLAYER_SIZE * i + 1 + INT_SIZE * 0] = pdata->players[i].player_rect.cords.x/100;
        buffer[PLAYER_SIZE * i + 1 + INT_SIZE * 0 + 1] = pdata->players[i].player_rect.cords.x%100;
        buffer[PLAYER_SIZE * i + 1 + INT_SIZE * 1] = pdata->players[i].player_rect.cords.y/100;
        buffer[PLAYER_SIZE * i + 1 + INT_SIZE * 1 + 1] = pdata->players[i].player_rect.cords.y%100;
...
void recv_structure(int sock, game_object_t* pdata)
{
    ...
    for (int i = 0; i < pdata->players_index; ++i)
    {
        //pdata->players[i].player_rect.cords.x = buffer[PLAYER_SIZE * i + 1 + INT_SIZE * 0];
        a = buffer[PLAYER_SIZE * i + 1 + INT_SIZE * 0];
        b = buffer[PLAYER_SIZE * i + 1 + INT_SIZE * 0 + 1];
        pdata->players[i].player_rect.cords.x = a*100+b;
        //pdata->players[i].player_rect.cords.y = buffer[PLAYER_SIZE * i + 1 + INT_SIZE * 1];
        a = buffer[PLAYER_SIZE * i + 1 + INT_SIZE * 1];
        b = buffer[PLAYER_SIZE * i + 1 + INT_SIZE * 1 + 1];
        pdata->players[i].player_rect.cords.y = a*100+b;
        pdata->players[i].player_rect.width   = buffer[PLAYER_SIZE * i + 1 + INT_SIZE * 2];
        pdata->players[i].player_rect.height  = buffer[PLAYER_SIZE * i + 1 + INT_SIZE * 3];
